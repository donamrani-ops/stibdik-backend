...
// ── Routes admin AVANT les routes avec :productId ────────────────────────────
// [Admin] Voir tous les abonnements actifs
router.get('/admin/all', protect, authorize('admin'), async (req, res, next) => {
  try {
    ...
    const subs = await StockNotification.find({ notified: false })
      .populate('product', 'nameFr stock')
      .lean();
    res.json({ success: true, count: subs.length, subscriptions: subs });
  } catch (err) { next(err); }
});

// [Admin] Déclencher manuellement les notifications pour un produit
router.post('/admin/trigger/:productId', protect, authorize('admin'), async (req, res, next) => {
  try {
    ...
    for (const sub of subs) {
      try {
        await emailService.sendRestockNotification(sub.email, sub.userName, product.nameFr, pid);
        sub.notified = true;
...
// Vérifier si des produits surveillés sont revenus en stock
